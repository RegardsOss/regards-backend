...
/**
 * Abstract class for building specifications for Feature Simple Entities Search.
 *
 * @author Thibaud Michaudel
 **/
public class AbstractFeatureSimpleEntitySpecificationBuilder<T>
    extends AbstractSpecificationsBuilder<T, SearchFeatureSimpleEntityParameters> {
    ...
    protected void addSpecificationsFromParameters(SearchFeatureSimpleEntityParameters parameters) {
        ...
        if (parameters.getDisseminationStatus() != null) {
            switch (parameters.getDisseminationStatus()) {
                case NONE -> add(isNull(DISSEMINATION_PENDING_FILED));
                case PENDING -> add(equals(DISSEMINATION_PENDING_FILED, true));
                case DONE -> add(equals(DISSEMINATION_PENDING_FILED, false));
                default -> {
                    // nothing to do
                }
            }
