 ...
 * <p>{@link FeatureNotificationRequest}s should be created after receiving AMQP events with retry.</p>
 *
 * @author Iliana Ghazali
 **/
@TestPropertySource(properties = { "spring.jpa.properties.hibernate.default_schema=feature_notification_handler_it",
                                   "regards.amqp.enabled=true" },
                    locations = { "classpath:retry.properties",
                                  "classpath:batch.properties",
                                  "classpath:metrics.properties" })
@ActiveProfiles(value = { "testAmqp", "noscheduler" })
public class FeatureNotificationRequestEventHandlerIT extends AbstractFeatureMultitenantServiceIT {

    @Autowired
    private IAmqpAdmin amqpAdmin;

    @MockBean
    private IFeatureNotificationRequestRepository featureNotificationRequestRepo;
    ...
    public void givenValidUpdateEvents_whenPublishedWithRetry_thenRequestsCreated() throws InterruptedException {
        ...
        Mockito.when(featureNotificationRequestRepo.saveAll(ArgumentMatchers.any()))
               .thenThrow(new DataAccessResourceFailureException("test exception to make the batch fail on "
                                                                 + "NOTIFICATION."))
               .thenAnswer(ans -> {
                   List<FeatureNotificationRequest> registeredRequests = (List<FeatureNotificationRequest>) ans.getArguments()[0];
        ...
        publisher.publish(prepareNotificationRequests(this.featureRepo.findAll()
                                                                      .stream()
