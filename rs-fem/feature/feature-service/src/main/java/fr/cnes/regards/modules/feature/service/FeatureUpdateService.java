...
public class FeatureUpdateService extends AbstractFeatureService<FeatureUpdateRequest> implements IFeatureUpdateService {
    ...
    public RequestInfo<FeatureUniformResourceName> registerRequests(List<FeatureUpdateRequestEvent> events) {
        ...
        Map<FeatureUniformResourceName, ILightFeatureEntity> sessionInfoByUrn =
                getSessionInfoByUrn(events.stream().map(event -> event.getFeature().getUrn()).collect(Collectors.toSet()));

        events.forEach(item -> prepareFeatureUpdateRequest(item, sessionInfoByUrn.get(item.getFeature().getUrn()), grantedRequests, requestInfo, existingRequestIds));

        // Batch save
        updateRepo.saveAll(grantedRequests);

        LOGGER.trace("------------->>> {} update requests registered in {} ms", grantedRequests.size(), System.currentTimeMillis() - registrationStart);
    ...
    public RequestInfo<FeatureUniformResourceName> registerRequests(FeatureUpdateCollection toHandle) {
        ...
        for (Feature feature : toHandle.getFeatures()) {
            toTreat.add(FeatureUpdateRequestEvent.build(toHandle.getRequestOwner(), toHandle.getMetadata(), feature, OffsetDateTime.now().minusSeconds(1)));
        }
     ...
     */
    private void prepareFeatureUpdateRequest(FeatureUpdateRequestEvent item, ILightFeatureEntity sessionInfo, List<FeatureUpdateRequest> grantedRequests,
        ...
        if (errors.hasErrors()) {
            // Monitoring log
            FeatureLogger.updateDenied(item.getRequestOwner(), item.getRequestId(), featureId, urn, ErrorTranslator.getErrors(errors));
            // Publish DENIED request
            publisher.publish(FeatureRequestEvent.build(FeatureRequestType.PATCH, item.getRequestId(), item.getRequestOwner(), featureId, urn,
                                                        RequestState.DENIED, ErrorTranslator.getErrors(errors)));
            if (item.getFeature() == null) {
                requestInfo.getMessages().add(String.format("Request %s without feature has been rejected", item.getRequestId()));
            } else {
            ...
            FeatureUpdateRequest request = FeatureUpdateRequest.build(item.getRequestId(), item.getRequestOwner(), item.getRequestDate(), RequestState.GRANTED, null,
                                                                      item.getFeature(), item.getMetadata().getPriority(), FeatureRequestStep.LOCAL_DELAYED);

            // Monitoring log
            FeatureLogger.updateGranted(request.getRequestOwner(), request.getRequestId(), request.getProviderId(), request.getUrn());
            // Publish GRANTED request
            publisher.publish(FeatureRequestEvent.build(FeatureRequestType.PATCH, item.getRequestId(), item.getRequestOwner(),
                                                        featureId, null, RequestState.GRANTED, null));
    ...
    public int scheduleRequests() {
        ...
        if (!requestsToSchedule.isEmpty()) {
            ...
            if (!requestsToSchedule.isEmpty()) {
                ...
                Map<FeatureUniformResourceName, ILightFeatureEntity> sessionInfoByUrn =
                        getSessionInfoByUrn(requestsToSchedule.stream().map(ILightFeatureUpdateRequest::getUrn).collect(Collectors.toSet()));
                ...
                LOGGER.trace("------------->>> {} update requests scheduled in {} ms", requestsToSchedule.size(), System.currentTimeMillis() - scheduleStart);
    ...
    public Set<FeatureEntity> processRequests(List<FeatureUpdateRequest> requests, FeatureUpdateJob featureUpdateJob) {
        ...
        Map<FeatureUniformResourceName, FeatureEntity> featureByUrn = this.featureRepo
                .findByUrnIn(requests.stream().map(FeatureUpdateRequest::getUrn).collect(Collectors.toList()))
                .stream()
        ...
        for (FeatureUpdateRequest request : requests) {
            ...
            if (entity == null) {
                ...
                request.addError(String.format("No feature referenced in database with following URN : %s", request.getUrn()));
                errorRequests.add(request);

                // Monitoring log
                FeatureLogger.updateError(request.getRequestOwner(), request.getRequestId(), request.getProviderId(), request.getUrn(), request.getErrors());
                // Publish request failure
                publisher.publish(FeatureRequestEvent.build(FeatureRequestType.PATCH, request.getRequestId(), request.getRequestOwner(), request.getProviderId(),
                ...
                if (entity.getFeature().getHistory() != null) {
                    ...
                    entity.getFeature().setHistory(FeatureHistory.build(request.getRequestOwner(), request.getRequestOwner()));
                }

                // Merge properties handling null property values to unset properties
                IProperty.mergeProperties(entity.getFeature().getProperties(), patch.getProperties(), patch.getUrn().toString(), request.getRequestOwner());
                ...
                // Monitoring log
                FeatureLogger.updateSuccess(request.getRequestOwner(), request.getRequestId(), request.getProviderId(), request.getFeature().getUrn());
                // Publish request success
                publisher.publish(FeatureRequestEvent.build(FeatureRequestType.PATCH, request.getRequestId(), request.getRequestOwner(), entity.getProviderId(),
