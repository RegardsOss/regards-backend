...
/**
 * Test for {@link IFeatureEntityRawClient}.
 *
 * @author Thibaud Michaudel
 */
@TestPropertySource(properties = { "spring.jpa.properties.hibernate.default_schema=fem_raw_client" })
@DirtiesContext(classMode = ClassMode.BEFORE_CLASS)
public class IFeatureEntityRawClientIT extends AbstractRegardsWebIT {

    private static final Logger LOG = LoggerFactory.getLogger(IFeatureEntityRawClientIT.class);

    @Value("${server.address}")
    private String serverAddress;

    private IFeatureEntityRawClient client;
    ...
    public void init() {
        ...
        client = FeignClientBuilder.build(new TokenClientProvider<>(IFeatureEntityRawClient.class,
    ...
    public void test_find_all() {
        ...
        ResponseEntity<PagedModel<EntityModel<FeatureEntityDto>>> response = client.findAllRaw("model",
                                                                                               OffsetDateTime.now()
                                                                                                             .minusDays(
                                                                                                                 2),
                                                                                               null,
    ...
    public void test_find_all_slice() {
        ...
        // THEN the first created features are returned, and has next link does not exist
        Assert.assertEquals(HttpStatus.OK, response.getStatusCode());
