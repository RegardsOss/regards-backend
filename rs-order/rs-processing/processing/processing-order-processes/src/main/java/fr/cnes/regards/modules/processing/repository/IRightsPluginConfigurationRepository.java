...
public interface IRightsPluginConfigurationRepository extends JpaRepository<RightsPluginConfiguration, Long> {
    ...
    @Query(value = " SELECT " + " DISTINCT ON (sub.id) " + "        sub.id," + "        sub.plugin_configuration_id, "
            + "        sub.process_business_id, " + "        sub.tenant, " + "        sub.user_role, "
            + "        sub.datasets, " + "        sub.is_linked_to_all_datasets " + " FROM ( "
            + "         SELECT rpc.*, unnest(array_append(rpc.datasets, '')) AS x "
            + "         FROM t_rights_plugin_configuration AS rpc " + " ) AS sub "
            + " WHERE sub.x = CAST(:dataset AS TEXT)" + "    OR sub.is_linked_to_all_datasets = TRUE"
