...
public class PExecutionRepositoryImplTest extends AbstractRepoTest {
    ...
    public void test_count_executions() throws Exception {
        ...
        // WHEN
        List<PExecution> execs = domainExecRepo.findAllForMonitoringSearch(
                batch.getTenant(),
                null, null,
                singletonList(SUCCESS),
                nowUtc().minusHours(5),
        ...
        Integer countRunning = domainExecRepo.countAllForMonitoringSearch(
                batch.getTenant(),
                batch.getProcessBusinessId().toString(),null,
        ...
        // WHEN
        List<PExecution> execRunningFirstPage = domainExecRepo.findAllForMonitoringSearch(
                batch.getTenant(),
                batch.getProcessBusinessId().toString(), batch.getUserEmail(),
                Arrays.asList(RUNNING, PREPARE, CLEANUP),
                nowUtc().minusHours(5),
                nowUtc().plusHours(5),
                PageRequest.of(0, 1)
        ).collectList().block();
        ...
        // WHEN
        List<PExecution> execRunningSecondPage = domainExecRepo.findAllForMonitoringSearch(
                batch.getTenant(),
                null, batch.getUserEmail(),
                Arrays.asList(RUNNING, PREPARE, CLEANUP),
                nowUtc().minusHours(5),
                nowUtc().plusHours(5),
                PageRequest.of(1, 1)
