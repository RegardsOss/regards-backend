...
public class IBatchEntityRepositoryTest extends AbstractRepoTest {

    private static final Logger LOGGER = LoggerFactory.getLogger(IBatchEntityRepositoryTest.class);

    @Test public void test_cleanable_batches() throws Exception {
        // GIVEN
    ...
    private void generateExecution(List<ExecutionEntity> entities, BatchEntity batchRecentExec, ExecutionStatus success, OffsetDateTime lastUpdated) {
        entities.add(randomInstance(ExecutionEntity.class).withBatchId(batchRecentExec.getId())
                .withTenant(batchRecentExec.getTenant()).withUserEmail(batchRecentExec.getUserEmail())
