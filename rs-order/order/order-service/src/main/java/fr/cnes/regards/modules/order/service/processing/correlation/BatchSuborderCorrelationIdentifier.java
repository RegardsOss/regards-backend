...
public class BatchSuborderCorrelationIdentifier {
    ...
    private static final Pattern PARSE_FORMAT = Pattern.compile(
        "order-(?<orderId>\\d+)_dsSel-(?<dsSelId>\\d+)_subOrder-(?<subOrderId>\\d+)");

    public static Option<BatchSuborderCorrelationIdentifier> parse(String repr) {
        Matcher matcher = PARSE_FORMAT.matcher(repr);
        if (matcher.matches()) {
            return Option.some(new BatchSuborderCorrelationIdentifier(Long.parseLong(matcher.group("orderId"), 10),
