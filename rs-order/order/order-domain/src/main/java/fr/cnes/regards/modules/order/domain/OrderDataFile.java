...
@NamedNativeQueries({ @NamedNativeQuery(query =
    "SELECT o.*, sum(df.size) as size FROM {h-schema}t_data_file df, {h-schema}t_order o "
    ...
    + "GROUP BY o.id ORDER BY o.id", resultSetMapping = "sumMapping", name = "selectSumSizesByOrderId"),
    @NamedNativeQuery(query =
        "SELECT o.*, sum(df.size) as size FROM {h-schema}t_data_file df, {h-schema}t_order o WHERE "
        + "df.order_id = o.id AND df.size is not NULL AND "
        + "o.id IN (SELECT id FROM {h-schema}t_order WHERE ?1 <= expiration_date AND "
        + "status in ('RUNNING', 'PAUSED')) "
        + "AND df.state IN (?2) GROUP BY o.id ORDER BY o.id", resultSetMapping = "sumMapping",
        name = "selectSumSizesByOrderIdAndStates"),
    @NamedNativeQuery( // WARNING : this request is used to count files in error (except DOWNLOAD_ERROR) so
        ...
        query = "SELECT o.*, count(df.*) as count FROM {h-schema}t_data_file df, {h-schema}t_order o WHERE "
                ...
                + "o.id IN (SELECT id FROM {h-schema}t_order WHERE ?1 <= expiration_date AND "
                + "status in ('RUNNING', 'PAUSED')) "
                + "AND df.state IN (?2) GROUP BY o.id ORDER BY o.id", resultSetMapping = "countMapping",
        name = "selectCountFilesByOrderIdAndStates"),
    @NamedNativeQuery( // WARNING : this request permits to count all available files EVEN  external files which
        ...
        query = "SELECT o.*, count(df.*) as count FROM {h-schema}t_data_file df, {h-schema}t_order o WHERE "
                ...
                + "AND df.state IN (?2) GROUP BY o.id ORDER BY o.id", resultSetMapping = "countMapping",
        name = "selectCountFilesByOrderIdAndStates4AllOrders") })
