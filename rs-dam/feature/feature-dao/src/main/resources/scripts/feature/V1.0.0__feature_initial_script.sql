create table t_feature (id int8 not null, urn varchar(132), session_owner varchar(128) NOT NULL, session_name varchar(128) NOT NULL, feature jsonb, last_update timestamp not null, provider_id varchar(100) not null, version numeric not null, primary key (id));
create table t_feature_creation_request (id int8 not null, provider_id varchar(100) not null, session_owner varchar(128) not null, session_name varchar(128) not null, storages jsonb, errors jsonb, request_id varchar(36) not null, state varchar(50) not null, feature jsonb not null, registration_date timestamp not null, request_date timestamp not null, step varchar(50) not null, priority numeric not null, group_id varchar(255), feature_id int8, primary key (id));
...
-- Update requests
create table t_feature_update_request (id int8 not null, provider_id varchar(100) not null, errors jsonb, registration_date timestamp not null, request_date timestamp not null, request_id varchar(36) not null, state varchar(50) not null, feature jsonb not null, group_id varchar(255), step varchar(50) not null, urn varchar(132) not null, feature_id int8, priority numeric not null, primary key (id));
...
create index idx_feature_update_step_registration_priority on t_feature_creation_request (step, registration_date, priority);
