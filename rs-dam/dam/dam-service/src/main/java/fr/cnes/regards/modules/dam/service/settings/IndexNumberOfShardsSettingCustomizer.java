...
@Component
public class IndexNumberOfShardsSettingCustomizer implements IDynamicTenantSettingCustomizer {
    ...
    public Errors isValid(DynamicTenantSetting dynamicTenantSetting) {
        ...
        if (!isProperValue(dynamicTenantSetting.getDefaultValue())) {
            errors.reject("invalid.default.setting.value",
                          "default setting value of parameter [number of shards] must be a valid number.");
        }
        if (dynamicTenantSetting.getValue() != null && !isProperValue(dynamicTenantSetting.getValue())) {
            errors.reject("invalid.setting.value",
                          "setting value of parameter [number of shards] can be null or must be a valid number.");
        }
        return errors;
    }
    ...
    public boolean appliesTo(DynamicTenantSetting dynamicTenantSetting) {
        return DamSettings.INDEX_NUMBER_OF_SHARDS.equals(dynamicTenantSetting.getName());
    }
