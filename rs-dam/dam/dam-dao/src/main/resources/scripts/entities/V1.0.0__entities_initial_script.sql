...
create table t_description_file (id int8 not null, description_file_content bytea, description_file_type varchar(255), url text, primary key (id));
create table t_entity (dtype varchar(10) not null, id int8 not null, creation_date timestamp not null, geometry jsonb, ipId varchar(128) not null, label varchar(128) not null, update_date timestamp, properties jsonb, sipId varchar(255), data_model_id int8, licence text, openSearchSubsettingClause text, score int4, sub_setting_clause jsonb, model_id int8 not null, description_file_id int8, ds_plugin_conf_id int8, primary key (id));
...
alter table t_entity add constraint fk_entity_description_file foreign key (description_file_id) references t_description_file;
