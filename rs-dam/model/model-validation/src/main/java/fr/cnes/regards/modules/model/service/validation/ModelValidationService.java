 ...
 */
@Service
public class ModelValidationService implements IModelValidationService {

    private static final Logger LOGGER = LoggerFactory.getLogger(ModelValidationService.class);

    @Autowired
    private IModelFinder modelFinder;

    @Override
    public Errors validate(String model, Set<AbstractProperty<?>> properties, List<Validator> extraValidators) {
        ...
        Map<String, AbstractProperty<?>> pptyMap = getPropertyMap(properties);
        ...
        for (ModelAttrAssoc modelAttrAssoc : modAtts) {
            checkModelAttribute(modelAttrAssoc, errors, pptyMap, toCheckProperties);
        }
     ...
     *
     * FIXME : manage alterable!!!!!!
     */
    protected void checkModelAttribute(ModelAttrAssoc modelAttrAssoc, Errors errors,
