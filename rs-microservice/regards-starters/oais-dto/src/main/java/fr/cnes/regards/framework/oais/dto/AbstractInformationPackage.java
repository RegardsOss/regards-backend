...
public class AbstractInformationPackage<ID> extends AbstractFeature<InformationPackageProperties, ID> {
    ...
    /**
     * Register the newly constructed and fulfilled {@link ContentInformationDto} and add it to the set of content
     * informations of
     ...
     */
    public <T extends AbstractInformationPackage<ID>> T withProvenanceInformationEvents(EventDto... events) {
     ...
     * @param locations references to the physical file. Use {@link OAISDataObjectLocationDto} build methods to create
     *                  location!
     */
     ...
     */
    public <T extends AbstractInformationPackage<ID>> T withEvents(EventDto... events) {
     ...
     * @param events events to add
     */
    public <T extends AbstractInformationPackage<ID>> T withEvents(Collection<EventDto> events) {
        Assert.notNull(events, "Collection of events cannot be null");
        return withEvents(events.toArray(new EventDto[0]));
    }
