 ...
 * LICENSE_PLACEHOLDER
 */
package fr.cnes.regards.framework.modules.jpa.jpa;
...
public class JpaUtilsTest {
    ...
    @Purpose("Check for classapth validity when there is instance database entities and multitenant database entities in the same classpath")
    @Test
    public void test() {

        final String errorMessage = "There should be an exception the classpath is invalid";
        try {
            DaoUtils.checkClassPath("fr.cnes.regards.framework.jpa.test.invalid");
        ...
        try {
            DaoUtils.checkClassPath("fr.cnes.regards.framework.jpa.test.invalid2");
            Assert.fail(errorMessage);
        } catch (final MultiDataBasesException e) {
            LOG.info(e.getMessage());
        }
        ...
        try {
            DaoUtils.checkClassPath("fr.cnes.regards.framework.jpa.test.invalid3");
            Assert.fail(errorMessage);
        } catch (final MultiDataBasesException e) {
            LOG.info(e.getMessage());
        }
