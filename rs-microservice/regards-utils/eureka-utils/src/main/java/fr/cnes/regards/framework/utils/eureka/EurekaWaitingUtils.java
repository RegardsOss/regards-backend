 ...
 **/
public final class EurekaWaitingUtils {
    ...
    private static boolean checkEndpointStatus(String endpointToWait) {
        ...
        try {
            ...
            if (userInfo != null && !userInfo.isEmpty()) {
                httpConnection.setRequestProperty("Authorization", "Basic " + userInfo);
            }
            httpConnection.setRequestMethod("GET");
            httpConnection.setInstanceFollowRedirects(false);
            ok = httpConnection.getResponseCode() == 200;
        } catch (MalformedURLException | URISyntaxException e) {
            LOGGER.error("The URL {} is malformed", endpointToWait, e);
        } catch (IOException e) {
            // Unreachable server, nothing to do
            LOGGER.error("Failed connecting to {}", endpointToWait, e);
        } finally {
    ...
    private static boolean canBeStarted(String eurekaApiUrl, List<String> servicesToWait) {
        ...
        } catch (IOException | HttpResponseException e) { //NOSONAR log or rethrow exception
            // logging level is purposely "info" and not "error"
            // since it is normal to wait for the avaibility of the service
    ...
    private static List<EurekaApplication> getEurekaApps(String eurekaApiUrl)
        ...
        try {
            ...
            if (connection.getResponseCode() == 200) {
                //rs-registry is up
                EurekaGetResponseDto responseDto = unmarshallResponse(connection.getInputStream());
                return responseDto.getApplications();
            } else {
                throw new HttpResponseException(String.format("Error while getting Eureka instances, got error code %d",
