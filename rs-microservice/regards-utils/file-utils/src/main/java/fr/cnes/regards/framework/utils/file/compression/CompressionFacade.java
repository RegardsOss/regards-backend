...
public class CompressionFacade {
     ...
     */
    public Vector<CompressManager> compress(CompressionTypeEnum pMode,
                                            File pInputDirectory,
                                            List<File> pFileList,
                                            File pZipFile,
                                            File pRootDirectory,
     ...
     * @throws CompressionException si l'un des paramètres n'est pas correct
     */
    public Vector<CompressManager> compress(CompressionTypeEnum mode,
                                            File inputDirectory,
    ...
    public void decompress(CompressionTypeEnum pMode, File pInputFile, File pOutputDirectory, Charset pCharset)
        ...
        if (!pInputFile.isFile()) {
            throw new FileAlreadyExistException(String.format("%s must be a file (not a directory)",
        ...
        if (!pOutputDirectory.isDirectory()) {
            throw new FileAlreadyExistException(String.format("%s must be a directory (not a file)",
                                                              pOutputDirectory.getAbsolutePath()));
        }
    ...
    private List<File> validateDirectoryForCompress(File pInputDirectory) throws CompressionException {
        ...
        if (!pInputDirectory.isDirectory()) {
            throw new FileAlreadyExistException(String.format("%s must be a directory (not a file)",
                                                              pInputDirectory.getAbsolutePath()));
        }
    ...
    private void validateFile(File pFile) throws CompressionException {
        ...
        if (!pFile.exists()) {
            throw new CompressionException(String.format("The file or directory %s does not exist",
                                                         pFile.getAbsoluteFile()));
        }

        if (!pFile.canRead()) {
            throw new CompressionException(String.format("The compression mode %s is not defined",
                                                         pFile.getAbsoluteFile()));
        }
    }
