...
public class RegardsHttpResponseSnippet extends HttpResponseSnippet {
    ...
    protected Map<String, Object> createModel(Operation operation) {
        ...
        cleanResponseBody(model);
        return model;
    }

    /**
     * If response body not useful, remove it for good Mustache template rendering
     */
    private void cleanResponseBody(Map<String, Object> model) {

        if (model.containsKey(RESPONSE_BODY)) {
            Object body = model.get(RESPONSE_BODY);
            if (body == null) {
                model.remove(RESPONSE_BODY);
            } else {
