...
public class OrderInputFileMetadataMapper extends AbstractMapper<OrderInputFileMetadata> {
    ...
    public Option<OrderInputFileMetadata> fromMap(Map<String, String> map) {
        return parseBoolean(map, INTERNAL).flatMap(internal -> parseUrn(map, FEATURE_ID).flatMap(urn -> parseString(map,
                                                                                                                    STORED_PATH).orElse(
            Option.some(null)).map(storedPath -> new OrderInputFileMetadata(internal, urn, storedPath))));
    }
