...
public abstract class AbstractRepoIT extends AbstractProcessingIT implements RandomUtils {

    protected static final Logger LOGGER = LoggerFactory.getLogger(AbstractRepoIT.class);

    @Autowired
    protected IPBatchRepository domainBatchRepo;

    @Autowired
    protected IPExecutionRepository domainExecRepo;

    @Autowired
    protected IPOutputFilesRepository domainOutputFilesRepo;

    @Autowired
    protected IBatchEntityRepository entityBatchRepo;

    @Autowired
    protected IExecutionEntityRepository entityExecRepo;

    @Autowired
    protected IOutputFileEntityRepository entityOutputFilesRepo;
    ...
    protected Mono<PBatch> saveBatch(int i, PBatch pBatch) {
        return domainBatchRepo.save(pBatch).doOnNext(b -> LOGGER.info("ATTEMPT {}, Saved batch {}", i, b));
    }

    protected Mono<PExecution> saveExec(int i, PExecution pExec) {
        return domainExecRepo.create(pExec).doOnNext(e -> LOGGER.info("ATTEMPT {}, Saved exec {}", i, e));
    }
    ...
    protected Mono<PExecution> addStep(int i, PExecution persistedExec, PStep step) {
        ...
        return domainExecRepo.update(executionWithNewStep)
                             ...
                             .doOnError(t -> LOGGER.error(
                                 "ATTEMPT {}, Failed to save exec with new step: persisted={}, version={}, exec={}",
                                 i,
                                 executionWithNewStep.isPersisted(),
                                 executionWithNewStep.getVersion(),
                                 executionWithNewStep,
                                 t));
    }

    protected Mono<PBatch> findBatch(int i, PBatch pBatch) {
        return domainBatchRepo.findById(pBatch.getId()).doOnNext(b -> LOGGER.info("ATTEMPT {}, Found batch {}", i, b));
    }

    protected Mono<PExecution> findExec(int i, PExecution pExec) {
        return domainExecRepo.findById(pExec.getId()).doOnNext(e -> LOGGER.info("ATTEMPT {}, Found exec {}", i, e));
    }

    protected static PExecution asNew(PExecution exec) {
        OffsetDateTime lastUpdated = nowUtc();
        return exec.withSteps(empty())
