...
public class IExecutableTest {
    ...
    IExecutable exec2e = context -> mkMono(tokens, context, "exec2").flatMap(c -> {
        tokens.add("error");
    ...
    public void andThen1() {
        ...
        exec123.execute(ctx).subscribe(x -> {
        }, t -> {
        });
    ...
    public void onError1() {
        IExecutable exec12e3 = exec1.andThen("exec2", exec2e // error occurs after exec2
                                                             .onError((c, t) -> { // immediate recovery
                                                                 tokens.add("recover");
                                                                 return Mono.just(c);
                                                             }).andThen("exec3", exec3));

        exec12e3.execute(ctx).subscribe(x -> {
        }, t -> {
        });
    ...
    public void onError2() {
        IExecutable exec12er3 = exec1.andThen("exec2e", exec2e) // error occurs after exec2
                                     .onError((c, t) -> { // recovery
                                         tokens.add("recover");
                                         return Mono.just(c);
                                     }).andThen("exec3", exec3);

        exec12er3.execute(ctx).subscribe(x -> {
        }, t -> {
        });
        assertThat(tokens).containsExactly("exec1", "exec2", "error", "recover", "exec3");
    }
    ...
    public void onError3() {
        IExecutable exec12e3r = exec1.andThen("exec2e", exec2e) // error occurs after exec2
                                     .andThen("exec3", exec3).onError((c, t) -> { // recovery after exec3
                tokens.add("recover");
                return Mono.just(c);
            });

        exec12e3r.execute(ctx).subscribe(x -> {
        }, t -> {
        });
    ...
    public void interrupt1() {
        ...
        exec12i3.execute(ctx).subscribe(x -> {
        }, t -> {
        });
