...
import fr.cnes.regards.modules.search.service.IAccessRightFilterService;
import fr.cnes.regards.modules.search.service.IFilterService;
 ...
 * <li>Applies project filters by interpreting the research criterion and transforming them in ElasticSearch request.
 * This is done with {@link IFilterService}.
...
public class SearchController {
    ...
    @Autowired
    private IAccessRightFilterService accessRightFilterService;
    ...
    public <T extends AbstractEntity> ResponseEntity<PagedResources<Resource<T>>> search(@PathVariable final String pQ,
            final Pageable pPageable, final PagedResourcesAssembler<T> pAssembler) throws ParseException {

        final Analyzer analyzer = new SimpleAnalyzer();
        final QueryParser parser = new QueryParser("name", analyzer);

        final Query query = parser.parse(pQ);

        // Translate the OpenSearch request into ICriterion
        // ICriterion criterion = filterService.parseRequest();
        final ICriterion criterion = ICriterion.all();
        final Class<AbstractEntity> pClass = AbstractEntity.class;
