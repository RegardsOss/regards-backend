...
public class CatalogDownloadControllerIT extends AbstractRegardsTransactionalIT {
    ...
    public void restDownloadFileOk() {
    	performDefaultGet(DOWNLOAD_AIP_FILE, 
        		customizer().expectStatusOk(), "Error message", 
        		AIP_ID_OK, "checksumOk");
    }
    
    @Test()
    public void restDownloadFileNotFound() {
    	performDefaultGet(DOWNLOAD_AIP_FILE, 
        		customizer().expectStatusNotFound(), "Error message", 
        		AIP_ID_OK, "checksumKo");
    }
    
    @Test()
    public void restDownloadFileForbidden() {
    	performDefaultGet(DOWNLOAD_AIP_FILE, 
        		customizer().expectStatusForbidden(), "Error message", 
        		AIP_ID_FAIL, "checksumKo");
    }
}
