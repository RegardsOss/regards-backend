...
import fr.cnes.regards.modules.storage.plugin.staf.event.CollectEvent;
import fr.cnes.regards.modules.storage.plugin.staf.event.CollectEventOffLine;
import fr.cnes.regards.modules.storage.plugin.staf.event.ICollectListener;
import fr.cnes.regards.modules.storage.plugin.utils.encryption.EncryptionException;
import fr.cnes.regards.modules.storage.plugin.utils.encryption.EncryptionFacade;
import fr.cnes.regards.modules.storage.plugin.utils.encryption.EncryptionTypeEnum;
...
public class STAFService {
    ...
    /**
     * Nom du projet utilisateur du STAF (a distinguer du nom du projet STAF qui peut avoir une valeur differente
     * suivant la declaration du compte)
     */
     ...
     * Constructeur.
     */
    public STAFService(STAFConfiguration pConfiguration) {
        mainSession = new STAFSession(pConfiguration);
        configuration = pConfiguration;
    }

    /**
     * Cette methode permet de designer quel est le projet courant.
     *
     * @param pProjectName
     *            Nom du projet.
     */
     ...
     */
    public void connectArchiveSystem(STAFArchive pSTAFArchive, ArchiveAccessModeEnum pMode) throws STAFException {
        // Recupere une reference locale au gestionnaire STAF et a la
        // configuration
        logger.debug("Connecting to STAF ...");
        final STAFManager manager = STAFManager.getInstance(configuration);
        try {
            // Recupere et decrypte le mot de passe
            final EncryptionFacade crypteur = new EncryptionFacade();
            final String clearPassword = crypteur.decrypt(EncryptionTypeEnum.AES, pSTAFArchive.getPassword());
            // Memorise la valeur des parametres de connexion pour permettre
            // l'ouverture ulterieure de sessions supplementaires
            stafProject = pSTAFArchive.getArchiveName();
            password = clearPassword;
            gfAccount = pSTAFArchive.isGFAccount();
            // Reserve une session et ouvre la connexion selon le mode :
            // restitution ou archivage
            mainSessionId = manager.getReservation(pMode);
    ...
    public void disconnectArchiveSystem(ArchiveAccessModeEnum pMode) throws STAFException {
        ...
        manager.freeReservation(mainSessionId, pMode);
    }
    ...
    public void restoreAllFiles(List<String> pStafFilePathList, String pDestination) throws STAFException {
        // Gestionnaire de ressources STAF
        final STAFManager manager = STAFManager.getInstance(configuration);
        ...
        if ((pStafFilePathList != null) && !pStafFilePathList.isEmpty()) {
            ...
            while (files.hasNext()) {
                ...
                sessionNb = pStafFilePathList.size() / configuration.getMaxSessionsRestitutionMode().intValue();
                if (sessionNb > 0) {
                    sessionsIdentifiers = manager.getReservations(sessionNb - 1, false,
                ...
                while ((streamIndex < configuration.getMaxSessionStreamsRestitutionMode().intValue())
                        && files.hasNext()) {
                    sessionIndex = 0;
                    while ((sessionIndex < sessionNb) && files.hasNext()) {
                        ...
                        fileIndex = 0;
                        while ((fileIndex < configuration.getMaxStreamFilesRestitutionMode().intValue())
                                && files.hasNext()) {
    ...
    private List<String> runSessionsStaffilArchive(List<Integer> pIdentifiers, List<STAFArchivingFlow> pFlowList,
        ...
        boolean error = false;
        final int nbMaxFlowSession = configuration.getMaxSessionStreamsArchivingMode().intValue();
    ...
    protected STAFBackgroundSessionArchive getBackgroundSessionArchive(Integer pSessionId, String pProject,
        ...
        final STAFBackgroundSessionArchive session = new STAFBackgroundSessionArchive(pSessionId, pProject, pPassword,
                pFilesFlow, pDirectory, pArchivedFilesList, pReplace, configuration);
        return session;
    }
    ...
    protected STAFBackgroundSessionRetrieve getBackgroundSessionRetrieve(Integer pSessionId, String pProject,
        ...
        final STAFBackgroundSessionRetrieve session = new STAFBackgroundSessionRetrieve(pSessionId, pProject, pPassword,
                pFiles, configuration, getCollectListener());
        return session;
    }
    ...
    public List<String> archiveFiles(Map<String, String> pFileMap, String pDirectory, boolean pReplace)
        ...
        final int maxStreamFiles = configuration.getMaxStreamFilesArchivingMode().intValue();
        if (configuration.getLittleFileClass() == null) {
            ...
            flowList.addAll(dispatchFilesInSeveralFlow(littleFileServiceClassMap, maxStreamFiles,
                                                       configuration.getLittleFileClass()));
        }
        if (configuration.getBiggerFileGenClass() == null) {
            ...
            flowList.addAll(dispatchFilesInSeveralFlow(biggerFileGenServiceClassMap, maxStreamFiles,
                                                       configuration.getBiggerFileGenClass()));
        }
        if (configuration.getBiggerFileGFClass() == null) {
            ...
            flowList.addAll(dispatchFilesInSeveralFlow(biggerFileGFServiceClassMap, maxStreamFiles,
                                                       configuration.getBiggerFileGFClass()));
        }
        ...
        final int maxSessionsStream = configuration.getMaxSessionStreamsArchivingMode().intValue();
        ...
        while (iter.hasNext()) {
            ...
            if (sessionNbNeed > 0) {
                sessionsIdentifiers = manager.getReservations(sessionNbNeed - 1, false,
    ...
    public Map<Integer, List<File>> prepareFiles(List<String> pFileList) throws STAFException {
        ...
        final long minStandardFileSize = configuration.getMinFileSize().longValue();
        final long maxStandardFileSize = configuration.getMaxFileSize().longValue();
