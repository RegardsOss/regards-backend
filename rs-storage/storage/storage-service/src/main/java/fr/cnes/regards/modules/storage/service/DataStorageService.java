...
public class DataStorageService implements IDataStorageService {
    ...
    public Collection<PluginStorageInfo> getMonitoringInfos() throws ModuleException, IOException {
        ...
        Set<PluginConfiguration> activeDataStorageConfs = dataStorageConfigurations.stream()
                .filter(PluginConfiguration::isActive).collect(Collectors.toSet());
        ...
        if (oData.isPresent()) {
            ...
            switch (type) {
                ...
                default:
                    ...
                    long nbMetadataFileStored = dataFileDao
                            .countByAipAndTypeAndState(data.getAip(), DataType.AIP, DataFileState.STORED);
                    ...
                    if (data.getState() == DataFileState.PARTIAL_DELETION_PENDING) {
                        ...
                        // in case we failed to delete metadata following an update (more than 1 StorageDataFile of type AIP in state STORED),
                        // the AIP state should not be updated.
                        if ((data.getDataType() != DataType.AIP) || (nbMetadataFileStored == 1)) {
                            AIP aip = data.getAip();
                    ...
                    if ((data.getDataType() == DataType.AIP) && (nbMetadataFileStored > 1)) {
                        // in case we failed to delete metadata following an update (more than 1 StorageDataFile of type AIP in state STORED),
                        // the metadata file is marked as deletion_error so we can recognize the true metadata file.
                        data.setState(DataFileState.DELETION_ERROR);
                    } else {
