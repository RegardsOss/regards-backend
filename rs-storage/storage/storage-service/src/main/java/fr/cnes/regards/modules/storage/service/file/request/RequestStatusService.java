...
public class RequestStatusService {
    ...
    public void checkDelayedStorageRequests() {
        int nbUpdated = 0;
        List<FileStorageRequest> undelayedRequests = new ArrayList<>();
        for (FileStorageRequest delayedRequest : storageReqRepo.findByStatus(FileRequestStatus.DELAYED,
            ...
            Optional<FileStorageRequest> sameRequest = undelayedRequests.stream()
                                                                        .filter(ur -> ur.getStorage().equals(storage)
                                                                                      && ur.getMetaInfo()
                                                                                           .getChecksum()
                                                                                           .equals(checksum))
                                                                        .findFirst();
            if (sameRequest.isEmpty()) {
                ...
                FileStorageRequest undelayedRequest = sameRequest.get();
