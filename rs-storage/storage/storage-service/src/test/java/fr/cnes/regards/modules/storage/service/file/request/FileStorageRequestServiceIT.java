...
public class FileStorageRequestServiceIT extends AbstractStorageIT {
    ...
    public void retryMultipleStoreErrors() throws InterruptedException, ExecutionException, ModuleException {
        ...
        // schedule and run job to initiate the storage job associated to the FileReferenceRequest created earlier.
        super.scheduleStorageRequestJob(1, FileRequestStatus.ERROR);
        ...
        Page<FileStorageRequestAggregation> fileRefReqs = storageRequestService.search(fileRefReq.getStorage(),
                                                                                       PageRequest.of(0,
                                                                                                      1000,
                                                                                                      Direction.ASC,
                                                                                                      "id"));
        Page<FileReference> fileRefs = referenceService.search(fileRefReq.getStorage(),
    ...
    public void retryMultipleStoreErrorsByOwner() throws InterruptedException, ExecutionException, ModuleException {
        ...
        Collection<JobInfo> jobs = storageRequestService.scheduleJobs(FileRequestStatus.ERROR, null, owners);
        ...
        // After storage job is successfully done, the FileRefenrece should be created and the FileReferenceRequest should be removed.
        Page<FileStorageRequestAggregation> fileRefReqs = storageRequestService.search(ONLINE_CONF_LABEL,
                                                                                       PageRequest.of(0,
                                                                                                      1000,
                                                                                                      Direction.ASC,
                                                                                                      "id"));
        Page<FileReference> fileRefs = referenceService.search(ONLINE_CONF_LABEL,
                                                               PageRequest.of(0, 1000, Direction.ASC, "id"));
    ...
    public void retryMultipleStoreErrorsByStorage() throws InterruptedException, ExecutionException, ModuleException {
        ...
        Collection<JobInfo> jobs = storageRequestService.scheduleJobs(FileRequestStatus.ERROR, storages, null);
        Assert.assertEquals("One storage job should scheduled", 1, jobs.size());
        // Run Job and wait for end
        String tenant = runtimeTenantResolver.getTenant();
        jobService.runJob(jobs.iterator().next(), tenant).get();
        runtimeTenantResolver.forceTenant(tenant);
        // After storage job is successfully done, the FileRefenrece should be created and the FileReferenceRequest should be removed.
        Page<FileStorageRequestAggregation> fileRefReqs = storageRequestService.search(PageRequest.of(0,
                                                                                                      1000,
                                                                                                      Direction.ASC,
                                                                                                      "id"));
        Page<FileReference> fileRefs = referenceService.search(PageRequest.of(0, 1000, Direction.ASC, "id"));
    ...
    public void retryStoreErrors() throws ModuleException {
        ...
        this.updatePluginConfForError("unknown.*");
        // Run Job schedule to initiate the storage job associated to the FileReferenceRequest created before
        Collection<JobInfo> jobs = storageRequestService.scheduleJobs(FileRequestStatus.ERROR, null, null);
        Assert.assertEquals("One storage job should scheduled", 1, jobs.size());
        // Run Job and wait for end
        runAndWaitJob(jobs);
        // After storage job is successfully done, the FileRefenrece should be created and the FileReferenceRequest should be removed.
        Collection<FileStorageRequestAggregation> storageReqs = storageRequestService.search(fileRefReq.getStorage(),
                                                                                             fileRefReq.getMetaInfo()
                                                                                                       .getChecksum());
        Optional<FileReference> oFileRef = referenceService.search(fileRefReq.getStorage(),
    ...
    public void storeImageWithOnlinePlugin()
        ...
        Optional<FileReference> oFileRef = referenceService.search(destination.getStorage(), checksum);
        Assert.assertTrue("File reference should have been created.", oFileRef.isPresent());
    ...
    public void storeWithNotHandledFiles() throws InterruptedException, ExecutionException, MalformedURLException {
        ...
        Optional<FileReference> fileRef = referenceService.search(ONLINE_CONF_LABEL, checksumNotHandled);
        ...
        Assert.assertEquals("File reference request should still exists has the file to store has not been handled",
                            1,
                            storageReqs.size());
        fileRef = referenceService.search(ONLINE_CONF_LABEL, checksumHandled);
        storageReqs = storageRequestService.search(ONLINE_CONF_LABEL, checksumHandled);
    ...
    public void storeWithInvalidUrl() throws MalformedURLException {
        ...
        FileReferenceMetaInfo fileMetaInfo = new FileReferenceMetaInfo(checksum,
                                                                       ...
                                                                       MediaType.APPLICATION_OCTET_STREAM);
        // Run file reference creation.
        storageRequestService.handleRequest("someone",
                                            SESSION_OWNER_1,
                                            SESSION_1,
                                            fileMetaInfo,
    ...
    public void storeFileNearlineWithPendingActionRemaining() {
        ...
        runAndWaitJob(jobs);
        Optional<FileReference> fileRef = referenceService.search(NEARLINE_CONF_LABEL, checksum);
    ...
    public void storeWithUnknownStorageLocation() throws MalformedURLException {
        ...
        FileReferenceMetaInfo fileMetaInfo = new FileReferenceMetaInfo("invalid_checksum",
                                                                       ...
                                                                       132L,
                                                                       MediaType.APPLICATION_OCTET_STREAM);
        URL originUrl = new URL(ORIGIN_URL);
        ...
        Optional<FileReference> oFileRef = referenceService.search(destination.getStorage(),
                                                                   fileMetaInfo.getChecksum());
        Collection<FileStorageRequestAggregation> storageReqs = storageRequestService.search(destination.getStorage(),
                                                                                             fileMetaInfo.getChecksum());
        Assert.assertFalse(
