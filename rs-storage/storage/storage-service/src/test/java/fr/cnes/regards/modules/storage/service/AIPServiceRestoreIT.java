...
public class AIPServiceRestoreIT extends AbstractRegardsTransactionalIT {
    ...
    private void initDb() throws Exception {
        ...
        Set<IPluginParam> parameters = IPluginParam
                .set(IPluginParam.build(LocalDataStorage.LOCAL_STORAGE_TOTAL_SPACE, 10000000),
        ...
        parameters = IPluginParam.set(IPluginParam.build(LocalDataStorage.LOCAL_STORAGE_TOTAL_SPACE, 10000000),
                                      IPluginParam.build(LocalDataStorage.BASE_STORAGE_LOCATION_PLUGIN_PARAM_NAME,
                                                         gson.toJson(baseStorageLocation)));
    ...
    public void loadUnavailableFilesTest() throws ModuleException {
        ...
        Assert.assertTrue("No file should be directly available after AIPService::makeFilesAvailable. Cause : files to load does not exists !",
                          response.getAlreadyAvailable().isEmpty());
        Assert.assertTrue("All files should be in error after AIPService::makeFilesAvailable. Cause : files to load does not exists !",
    ...
    public void loadOnlineFilesTest() throws MalformedURLException, ModuleException {
        ...
        AvailabilityRequest request = new AvailabilityRequest(OffsetDateTime.now().plusDays(10), "1", "2", "3");
        AvailabilityResponse response = aipService.makeFilesAvailable(request);
        Assert.assertEquals("All files should be directly available after AIPService::makeFilesAvailable. Cause : files to load are online.",
