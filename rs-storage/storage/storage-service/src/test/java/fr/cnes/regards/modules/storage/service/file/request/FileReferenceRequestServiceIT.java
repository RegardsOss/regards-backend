...
public class FileReferenceRequestServiceIT extends AbstractStorageIT {
    ...
    public void referenceFileDuringDeletion() throws InterruptedException, ExecutionException {
        ...
        FileDeletionDto request = FileDeletionDto.build(fileRefChecksum,
                                                        ...
                                                        SESSION_1,
                                                        false);
        deletionRequestService.handle(List.of(request), deletionReqId);

        Optional<FileReference> oFileRef = referenceService.search(fileRefStorage, fileRefChecksum);
        Assert.assertFalse("File reference should no have any owners anymore",
                           referenceService.hasOwner(oFileRef.get().getId()));

        // Simulate FileDeletionRequest in PENDING state
        FileDeletionRequest fdr = deletionRequestRepository.findByFileReferenceId(fileRef.getId()).get();
        fdr.setStatus(FileRequestStatus.PENDING);
        deletionRequestRepository.save(fdr);
        ...
        Optional<FileDeletionRequest> ofdr = deletionRequestRepository.findByFileReferenceId(fdr.getId());
        ...
        Collection<FileStorageRequestAggregation> storageReqs = storageRequestService.search(fileRefStorage,
                                                                                             fileRefChecksum);
        ...
        oFileRef = referenceService.search(fileRefStorage, fileRefChecksum);
        ...
        FileDeletionJobProgressManager manager = new FileDeletionJobProgressManager(deletionRequestService,
                                                                                    storageLocationService,
                                                                                    new FileDeletionRequestJob());
        manager.deletionSucceed(fdr.toDto());
        referenceEventHandler.handleBatch(List.of(new FileReferenceEvent(fileRefChecksum,
                                                                         fileRefStorage,
        ...
        storageReqs = storageRequestService.search(fileRefStorage, fileRefChecksum);
        ...
        Assert.assertTrue("File reference should still exists", oFileRef.isPresent());
        FileReference fr = referenceWithOwnersRepository.findOneById(oFileRef.get().getId());
    ...
    public void referenceFileWithInvalidURL() {
        ...
        try {
            referenceRequestService.reference("someone",
                                              fileMetaInfo,
                                              location,
