...
create table t_file_cache_request (id int8 not null, checksum varchar(128) not null, error_cause varchar(512), expiration_date timestamp, file_size int8 not null, group_id varchar(128) not null, destination_path varchar(2048) not null, status varchar(255) not null, storage varchar(128) not null, file_ref_id int8 not null, primary key (id));
...
create table t_groups_requests_info (id int8 not null, checksum varchar(128) not null, error boolean, error_cause varchar(512), group_id varchar(128) not null, type varchar(255) not null, storage varchar(128), file_ref_id int8, primary key (id));
...
create index idx_group_id on t_groups_requests_info (group_id);
alter table t_groups_requests_info add constraint uk_t_groups_requests_info unique (group_id, file_ref_id);
...
alter table t_groups_requests_info add constraint FK278nmeardgw7tpeivopyrrct0 foreign key (file_ref_id) references t_file_reference;
