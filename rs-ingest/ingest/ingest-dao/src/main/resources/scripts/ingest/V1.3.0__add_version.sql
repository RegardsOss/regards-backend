...
UPDATE t_aip a1 SET last = false WHERE a1.id IN (SELECT a2.id FROM t_sip a2 WHERE a2.provider_id = a1.provider_id AND a2.version <> (SELECT max(a3.version) FROM t_sip a3 WHERE a3.provider_id = a1.provider_id AND a3.state <> 'DELETED') );
