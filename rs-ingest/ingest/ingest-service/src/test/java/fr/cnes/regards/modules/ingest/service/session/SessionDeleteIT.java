...
public class SessionDeleteIT extends IngestMultitenantServiceIT {
    ...
    public void testDeleteSource() {
        ...
        Page<AIPEntity> aipsDeleted = aipRepository.findAll(AIPEntitySpecification.searchAll(
                SearchAIPsParameters.build().withSessionOwner(SOURCE_1), PageRequest.of(0, 10)), PageRequest.of(0, 10));
        Assert.assertEquals("AIPs should have been deleted", 0, aipsDeleted.getContent().size());

        // test aips linked to SOURCE 2 are present
        Page<AIPEntity> aips = aipRepository.findAll(AIPEntitySpecification.searchAll(
                SearchAIPsParameters.build().withSessionOwner(SOURCE_2), PageRequest.of(0, 10)), PageRequest.of(0, 10));
    ...
    public void testSessionDelete() {
        ...
        Page<AIPEntity> aipsDeleted = aipRepository.findAll(AIPEntitySpecification.searchAll(
                SearchAIPsParameters.build().withSessionOwner(SOURCE_1).withSession(SESSION_1), PageRequest.of(0, 10)),
                                                            PageRequest.of(0, 10));
        Assert.assertEquals(0, aipsDeleted.getContent().size());

        // test aips linked to SESSION 2 SOURCE 1 are present
        Page<AIPEntity> aips1 = aipRepository.findAll(AIPEntitySpecification.searchAll(
                SearchAIPsParameters.build().withSessionOwner(SOURCE_1).withSession(SESSION_2), PageRequest.of(0, 10)),
                                                      PageRequest.of(0, 10));
        ...
        Page<AIPEntity> aips2 = aipRepository.findAll(AIPEntitySpecification.searchAll(
                SearchAIPsParameters.build().withSessionOwner(SOURCE_2).withSession(SESSION_1), PageRequest.of(0, 10)),
                                                      PageRequest.of(0, 10));
    ...
    private void initData() {
        ...
        publishSIPEvent(create("1", Lists.newArrayList("TAG_0")), "STORAGE_1", SESSION_1, SOURCE_1,
                        Lists.newArrayList("CATEGORIES_0"));
        publishSIPEvent(create("1", Lists.newArrayList("TAG_0")), "STORAGE_1", SESSION_2, SOURCE_1,
                        Lists.newArrayList("CATEGORIES_0"));
        publishSIPEvent(create("2", Lists.newArrayList("TAG_0")), "STORAGE_1", SESSION_1, SOURCE_2,
                        Lists.newArrayList("CATEGORIES_0"));
        // Wait
