create table t_account (id int8 not null, authentication_failed_counter int8, email varchar(100), external boolean, firstName varchar(100), invalidityDate timestamp, lastName varchar(100), password varchar(200), password_update_date timestamp, status varchar(10), primary key (id));
...
create table t_verification_token (id int8 not null, expiry_date timestamp, origin_url varchar(255), request_link varchar(255), token varchar(255), verified boolean, account_id int8 not null, primary key (id));
...
alter table t_verification_token add constraint uk_verification_token_account_id unique (account_id);
...
alter table t_verification_token add constraint fk_verification_token foreign key (account_id) references t_account;
