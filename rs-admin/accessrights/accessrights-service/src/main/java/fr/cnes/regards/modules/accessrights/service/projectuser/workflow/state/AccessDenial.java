...
/**
 * Action to deny the user access to the current project.
 * The user is currently waiting for access.
 * The user is now deny.
 *
 * @author Thomas Fache
 **/
public class AccessDenial extends AbstractUserAccessUpdate {

    public AccessDenial(IProjectUserRepository projectUserRepository,
    ...
    protected List<UserStatus> expectedAccesses() {
        return Collections.singletonList(UserStatus.WAITING_ACCESS);
    }
    ...
    protected Optional<ApplicationEvent> eventToPublish() {
        return Optional.of(new OnDenyEvent(user));
    }
}