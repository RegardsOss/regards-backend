...
public class SessionServiceTest extends AbstractMultitenantServiceTest {
    ...
    public void testUpdateState() throws ModuleException {
        ...
        Assert.assertEquals("Should state be updated to Acknowledged", sessionUpdated.getState(), SessionState.ACKNOWLEDGED);
    }
    ...
    public void testDelete() throws ModuleException {
        ...
        Assert.assertEquals("When deleted with force=false, the session state is DELETED", SessionState.DELETED, sessionOpt.get().getState());
    }
    ...
    public void testForceDelete() throws ModuleException {
        ...
        Optional<Session> sessionOpt = sessionRepository.findById(session.getId());
        Assert.assertFalse("When deleted with force=true, the session should not exist anymore", sessionOpt.isPresent());
    }
    ...
    public void testUpdateSessionProperty() {
        ...
        long value = 10;
        Assert.assertEquals("Check there is no session saved on DB before running the test", 0, sessionRepository.count());

        SessionMonitoringEvent sessionMonitoringEvent = SessionMonitoringEvent.build(
                source,
                name,
        ...
        Assert.assertEquals("The value is correctly saved in the session", value, updateSession.getStepPropertyValue(step, property));
        ...
        Assert.assertEquals("The session state is OK", SessionState.OK, updateSession.getState());
        Assert.assertEquals("The value is correctly saved in the session", 2 * value, updateSession.getStepPropertyValue(step, property));

        long decValue = 5;
        sessionMonitoringEvent = SessionMonitoringEvent.build(
                source,
                name,
                SessionNotificationState.OK,
                step,
        ...
        updateSession = sessionService.updateSessionProperty(sessionMonitoringEvent);
        Assert.assertEquals("The session state is OK", SessionState.OK, updateSession.getState());
        Assert.assertEquals("The value is correctly saved in the session", 15L, updateSession.getStepPropertyValue(step, property));
        ...
        Assert.assertEquals("The value is correctly saved in the session", -10L, updateSession.getStepPropertyValue(step, property));
        ...
        Assert.assertEquals("The session state is still in ERROR", SessionState.ERROR, updateSession.getState());
        Assert.assertEquals("The value have been replaced", einstein, updateSession.getStepPropertyValue(step, property));
        ...
        Assert.assertEquals("The session state is still in ERROR", SessionState.ERROR, updateSession.getState());
        Assert.assertEquals("The value is replaced by the value", value, updateSession.getStepPropertyValue(step, property));
    }
