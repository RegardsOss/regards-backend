...
export default class CreateAttributeModal extends React.Component<CreateAttributeModalProps, any> {
  ...
  render (): JSX.Element {
    const {label, type} = this.state
    const title = this.context.intl.formatMessage({id: "datamanagement.datasourcemodel.add.modal.header"})
    ...
    if (label.length > 0 && type !== 0) {
      actions.push(<FlatButton
        label={<FormattedMessage id="datamanagement.datasourcemodel.add.modal.action.create_and_close" />}
        primary={true}
        onTouchTap={this.handleAddAndClose}
      />)
      actions.push(<FlatButton
        label={<FormattedMessage id="datamanagement.datasourcemodel.add.modal.action.create_and_reset" />}
        primary={true}
    ...
    return (
      <div>
        ...
        >
          <TextField
            type="text"
            floatingLabelText={<FormattedMessage id="datamanagement.datasourcemodel.add.modal.input.name" />}
            value={label}
            fullWidth={true}
            onChange={this.handleAttributeLabelChange}
          />
          <SelectField
            floatingLabelText={<FormattedMessage id="datamanagement.datasourcemodel.add.modal.input.type" />}
            value={type}
            fullWidth={true}
