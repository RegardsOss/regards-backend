...
create table t_acq_processing_chain (id int8 not null, active boolean, ingest_chain varchar(255), label varchar(64) not null, last_activation_date timestamp, locked boolean not null, mode varchar(16), period int8, session varchar(50), generatesip_conf_id int8, acq_job_info_id uuid, postprocesssip_conf_id int8, product_conf_id int8, validation_conf_id int8, primary key (id));
create table t_acquisition_file (id int8 not null, acquisition_date timestamp, checksum varchar(255), checksumAlgorithm varchar(16), error text, filePath varchar(255), state varchar(32), acq_file_info_id int8, product_id int8, primary key (id));
create table t_acquisition_product (id int8 not null, error text, ip_id varchar(128), last_update timestamp not null, product_name varchar(128), session varchar(128), json_sip jsonb, sip_state varchar(32) not null, product_state varchar(32) not null, post_prod_job_info_id uuid, sip_gen_job_info_id uuid, sip_submission_job_info_id uuid, processing_chain_id int8, primary key (id));
...
alter table t_acq_processing_chain add constraint fk_acq_job_info_id foreign key (acq_job_info_id) references t_job_info;
...
alter table t_acquisition_product add constraint fk_post_prod_job_info_id foreign key (post_prod_job_info_id) references t_job_info;
alter table t_acquisition_product add constraint fk_sip_gen_job_info_id foreign key (sip_gen_job_info_id) references t_job_info;
alter table t_acquisition_product add constraint fk_sip_submission_job_info_id foreign key (sip_submission_job_info_id) references t_job_info;
