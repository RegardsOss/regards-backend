...
create table t_acq_processing_chain (id int8 not null, active boolean, categories jsonb, ingest_chain varchar(255), label varchar(64) not null, last_activation_date timestamp, locked boolean not null, mode varchar(16), period varchar(255), storages jsonb, generatesip_conf_id int8 not null, acq_job_info_id uuid, postprocesssip_conf_id int8, product_conf_id int8 not null, validation_conf_id int8 not null, primary key (id));
create table t_acquisition_file (id int8 not null, acquisition_date timestamp, error text, filePath varchar(255), state varchar(32), acq_file_info_id int8, product_id int8, primary key (id));
...
create index idx_acq_file_state on t_acquisition_file (state);
create index idx_acq_file_state_file_info on t_acquisition_file (state, acq_file_info_id);
create index idx_acq_file_info on t_acquisition_file (acq_file_info_id);
