...
public class IngestClientMock implements IIngestClient {
    ...
    private long waitingMillis = 0;

    @Override
    public ResponseEntity<Collection<SIPDto>> ingest(SIPCollection sips) {

        // Wait n seconds before SIP submission (DO NOT USE THREAD SLEEP, it will clean interrupted status)
        long startWaitingDate = System.currentTimeMillis();
        long endWaitingDate = startWaitingDate + waitingMillis;
    ...
    private SIPDto buildDto(SIP sip, Errors errors) {
        ...
        if (errors.hasErrors()) {
            dto.setState(SIPState.REJECTED);
    ...
    /**
     * Allow to slow down job execution
     * @param waitingMillis waiting time in milliseconds
     */
