...
public class SessionNotifier {
    ...
    public void notifyProductStateChange(SessionChangingStateProbe probe) {
        ...
        if (probe.isSessionChanged()) {
            notifyDecrementSession(
                    probe.getIngestionChain(), probe.getInitialSession(),
        ...
        if (probe.shouldUpdateState()) {
            ...
            if (probe.getInitialProductState() != null) {
                notifyDecrementSession(probe.getIngestionChain(), probe.getSession(),
            ...
            switch (probe.getProductState()) {
                ...
                case COMPLETED:
                case FINISHED:
                    notifyIncrementSession(probe.getIngestionChain(), probe.getSession(),
    ...
    public void notifyRelaunchSIPGeneration(Product product) {
        ...
        if (product.getSipState() != ProductSIPState.NOT_SCHEDULED) {
            ...
            notifyDecrementSession(product.getProcessingChain().getLabel(), product.getSession(), product.getSipState());
            // Add to submitting
            notifyIncrementSession(product.getProcessingChain().getLabel(), product.getSession(), ProductState.COMPLETED);
        }
    }
    ...
    public void notifyProductIngested(Product product) {
        ...
        notifyDecrementSession(product.getProcessingChain().getLabel(), product.getSession(), ProductSIPState.SUBMITTED);
        // Add to submitting
    ...
    public void notifyProductIngestFailure(Product product) {
        ...
        notifyIncrementSessionWithError(product.getProcessingChain().getLabel(), product.getSession(), product.getSipState());
    }
    ...
    private void notifyIncrementSessionWithError(String sessionOwner, String session, ISipState sipState) {
        notifyIncrementSession(sessionOwner, session, getProductStateProperty(sipState), SessionNotificationState.ERROR, 1L);
    }
    ...
    private void notifyIncrementSession(String sessionOwner, String session, String property, SessionNotificationState notifState, long nbItems) {
    ...
    private void notifyDecrementSession(String sessionOwner, String session, String property, SessionNotificationState notifState, long nbItems) {
        // Add one to the new state
        SessionMonitoringEvent event = SessionMonitoringEvent.build(
                sessionOwner,
                session,
